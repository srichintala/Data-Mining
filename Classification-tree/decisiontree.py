...
def calculate_accuracy(test_class_label_list,predicted_list):
    ...
    fold_error = float(false_predictions)/len(predicted_list)
    return fold_accuracy, fold_error
...
def make_predictions(test_feature_list,test_class_label_list,created_tree):
    ...
    fold_accuracy, fold_error = calculate_accuracy(test_class_label_list,predicted_list)
    accuracy_count = accuracy_count + fold_accuracy
    return accuracy_count, fold_error
...
def create_classficiation_tree(data_dictionary,class_labels,split_measure):
    ...
    #otherwise proceed to get best features and split the tree
    if not data_dictionary:
        majority_class_list = get_majority_value(class_labels)
        majority_class_label = majority_class_list[0][0]
        ...
        parent_class_count = get_majority_value(class_labels)
        left_class_count = get_majority_value(left_class_labels)
        right_class_count = get_majority_value(right_class_labels)
        #pruning the tree
        if(int(left_class_count[0][1])+int(right_class_count[0][1]) < int(parent_class_count[0][1])):
            root_node = Node(value_at_node,best_split_value,best_left_indices,best_right_indices,True)
            return root_node
        else:
...
def create_train_and_test_set(normalized_data, split_measure):
    ...
    for kfold in range(10):
        ...
        fold_accuracy, fold_error = make_predictions(test_feature_list[kfold],test_class_label_list[kfold],created_tree)
        accuracy_count = accuracy_count + fold_accuracy
        fold_count = fold_count + fold_error
        print "This is fold number", kfold
    total_accuracy = float(accuracy_count)
    total_error = float(fold_count)
    f1_measure = 2 * (float(accuracy_count) * float(fold_count))/(float(accuracy_count) + float(fold_count))
    print "Calculated accuracy after 10 fold cross validation is",float(total_accuracy)/10
    print "Calulated balanced sample accuracy after 10 fold cross validation is", 0.5* (float(total_error))/10
    print "Calulated F-1 measure after 10 fold cross validation is", f1_measure
